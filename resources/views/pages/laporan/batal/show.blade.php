			...
			<div class="row">
				...
				<div class="col-md-3">
					...
					@foreach($dt['transactionlogs'] as $key => $value)
						<p><strong>{{$value['status']}}</strong>&nbsp;&nbsp;&nbsp; @date_indo(new Carbon($value['changed_at'])) &nbsp; @if(in_array($value['status'], ['wait', 'veritrans_processing_payment', 'paid', 'shipping', 'delivered', 'canceled'])) <a href="{{route('shop.resend.email', ['id' => $value['transaction_id'], 'status' => $value['status']])}}">[ Resend Email ]</a> @endif</p>
					@endforeach
				</div>
			</div>
