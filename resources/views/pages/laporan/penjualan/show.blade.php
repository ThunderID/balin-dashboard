			...
			<div class="row">
				...
				<div class="col-md-3">
					...
					@foreach($dt['transactionlogs'] as $key => $value)
						<p>
							<strong>{{str_replace('_', ' ', $value['status'])}}</strong>&nbsp;&nbsp;&nbsp; 
							@date_indo(new Carbon($value['changed_at'])) &nbsp; 
							@if(in_array($value['status'], ['wait', 'paid', 'shipping', 'delivered', 'canceled'])) 
								<a href="{{route('shop.resend.email', ['id' => $value['transaction_id'], 'status' => $value['status']])}}">[ Resend Email ]</a> 
							@endif
							@if(in_array($value['status'], ['veritrans_processing_payment'])) 
								<a href="{{route('shop.resend.email', ['id' => $value['transaction_id'], 'status' => 'wait'])}}">[ Resend Email ]</a> 
							@endif
						</p>
					@endforeach
				</div>
